  ...
  describe "import_from_udata" do
    test "import dataset with a zip" do
      ...
      use_cassette "client/datasets/stan" do
        assert {:ok, dataset} = ImportDataService.import_from_udata("arrets-horaires-et-parcours-theoriques-du-reseau-stan-gtfs", "public-transit")
    ...
    test "import dataset with GTFS format" do
      ...
      use_cassette "client/datasets/metz" do
        assert {:ok, dataset} = ImportDataService.import_from_udata("transport-donnees-gtfs", "public-transit")
        assert List.first(dataset["resources"])["url"] == url
      end
    end
    ...
    test "import dataset with CSV" do
      ...
      use_cassette "client/datasets/sncf" do
        assert {:ok, dataset} = ImportDataService.import_from_udata("horaires-des-lignes-ter-sncf", "public-transit")
        assert List.first(dataset["resources"])["url"] == url
      end
    end
