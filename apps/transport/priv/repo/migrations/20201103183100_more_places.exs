defmodule DB.Repo.Migrations.MorePlaces do
  use Ecto.Migration
  ...
  def down do
    execute("DROP TRIGGER refresh_places_region_trigger ON region;")
    execute("DROP TRIGGER refresh_places_aom_trigger ON aom;")
    execute("DROP TRIGGER refresh_places_commune_trigger ON commune;")
    execute("DROP TRIGGER refresh_places_resources_trigger ON resource;")
    execute("DROP FUNCTION refresh_places;")
    execute("DROP MATERIALIZED VIEW places;")

    # migration from 20200224093551_places_mat_view.exs
    execute("""
  CREATE MATERIALIZED VIEW places AS
  SELECT nom, place_id, type, indexed_name
  FROM
