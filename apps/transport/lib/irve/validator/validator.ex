defmodule Transport.IRVE.Validator do
  ...
  def validate(path) do
    ...
    # probe: deviner le séparateur. Si ;, corriger en , et noter. Sinon, lever une erreur et arrêter
    # probe: vérifier les colonnes. Plein de possibilités. Colonnes qui manquent?  Colonnes en trop? Doublons?
    # (note: faire un profiling global de toutes les ressources dispos sur ce sujet, voir ce qui est utile à implémenter)
    # (on peut commencer strict : il faut "au moins exactement toutes les colonnes", et on droppe le reste avec un warning)
    # processing: supprimer les whitespaces (idéalement noter quand c'est fait avec un bitmask) en heading/trailing
