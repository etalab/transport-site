defmodule TransportWeb.ResourceControllerTest do
  ...
  describe "download" do
    ...
    test "download a PAN resource, valid token", %{conn: conn} do
      ...
      assert [%DB.ResourceDownload{token_id: ^token_id, resource_id: ^resource_id}] =
               DB.ResourceDownload |> DB.Repo.all()
    end
  ...
  describe "Validation reports" do
      ...
      for params <- gtfs_params("MissingCoordinates") do
        ...
        DB.Repo.update!(
          Ecto.Changeset.change(metadata, %{
            metadata: %{"networks_start_end_dates" => nil, "networks" => ["foo", "bar"]}
          })
        )
    ...
    test "NeTEx validation is shown", %{conn: conn} do
      ...
      for version <- ["0.1.0", "0.2.0", "0.2.1", "0.2.2"],
        ...
        if version in ["0.2.0", "0.2.1", "0.2.2"] do
