defmodule DB.IRVEValidFilePDCTest do
  ...
  test "all IRVE static schema fields are present" do
    ...
    assert official_schema_fields ==
             (ecto_schema_fields -- ["id", "irve_valid_file_id", "inserted_at", "updated_at"])
             |> Enum.map(fn field -> if field == "coordonneesxy", do: "coordonneesXY", else: field end)
