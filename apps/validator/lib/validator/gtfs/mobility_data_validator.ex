defmodule Validator.Gtfs.MobilityDataValidator do
  ...
  """
  def new(gtfs_path, output_dir),
    do: %__MODULE__{gtfs_path: gtfs_path, output_dir: output_dir}
  ...
  defp execute_gtfs_validator(
    ...
    {command_output, exit_code} =
      System.cmd(
        "java", ["-jar", gtfs_validator_bin(), "-o", output_dir, "-i", gtfs_path, "-f", "fr-test" ],
        into: [],
  ...
  # TODO get the bin directory from the config
  defp gtfs_validator_bin_directory,
    do: "/Users/lionel/Devs/projets/etalab/vendors/mobility_data/"

  defp update_validation_status(%__MODULE__{exit_code: 1} = validation), do: Map.put(validation, :status, :failed)

  defp update_validation_status(%__MODULE__{system_errors: %{"notices" => notices}} = validation) when length(notices) > 0,
       do: Map.put(validation, :status, :failed)

  defp update_validation_status(%__MODULE__{report: %{"notices" => notices}} = validation) when length(notices) > 0 do
    severities =
      notices
