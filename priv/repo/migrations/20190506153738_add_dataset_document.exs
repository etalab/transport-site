defmodule Transport.Repo.Migrations.AddDatasetDocument do
  ...
  def change do
    ...
    execute("""
        ...
        NEW.search_vector = setweight(to_tsvector(coalesce(NEW.spatial, '')), 'A') ||
              setweight(to_tsvector(array_to_string(NEW.tags, ',')), 'B') ||
        ...
        ELSIF NEW.region_id IS NOT NULL THEN
          SELECT region.nom INTO nom FROM region where region.id = NEW.region_id;
          NEW.search_vector = NEW.search_vector ||
            setweight(to_tsvector(coalesce(nom, '')), 'A');
        END IF;
